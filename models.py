...
class FasNet:
    ...
    def __dense(self, feature):
        ...
        feature = tf.keras.layers.Activation(
            activation=tf.nn.leaky_relu)(feature)
    ...
    def __create_model(self):
        input_hsv = tf.keras.layers.Input(name='hsv_input',
                                          shape=(self.input_size,
                                                 self.input_size, 3))
        input_yuv = tf.keras.layers.Input(name='yuv_input',
                                          shape=(self.input_size,
                                                 self.input_size, 3))

        vgg_hsv = tf.keras.applications.VGG16(
            weights=self.fine_tune_model_file, include_top=False)
        vgg_yuv = tf.keras.applications.VGG16(
            weights=self.fine_tune_model_file, include_top=False)
        ...
        output = tf.keras.layers.Dense(name='output',
                                       units=self.class_num,
                                       activation=tf.nn.softmax)(feature)
    ...
    def predict(self):
        ...
        predictions = self.model.predict_generator(
            generator=self.dataset.test_data_generator(input_name_list,
                                                       output_name_list,
                                                       'test.txt',
                                                       shuffle=False),
        ...
        print("fas==> correct:{}, total:{}, correct_rate:{}".format(
    ...
    def test_online(self, face_imgs):
        ...
        predictions = self.model.predict(
            {
                'hsv_input': batch_x_hsv,
                'yuv_input': batch_x_ycrcb
            },
            batch_size=1)
